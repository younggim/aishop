export default function Footer() {
  return (
    <footer className="mt-auto border-t border-zinc-100 bg-zinc-50">
      <div className="mx-auto flex max-w-6xl flex-col gap-2 px-4 py-8 text-xs text-zinc-500">
        <p className="font-semibold text-zinc-700">AI Device Mall</p>
        <p>대한민국 NO.1 AI 디바이스 멀티채널 커머스 플랫폼</p>
        <p>사업자 정보 · 이용약관 · 개인정보처리방침 · 고객센터 1588-0000</p>
        <p>
          담당자 Jaeyoung Kim ·{" "}
          <a href="mailto:jaeyoungai1@gmail.com" className="hover:text-zinc-700 hover:underline">
            jaeyoungai1@gmail.com
          </a>
        </p>
        <p>© 2026 AI Device Mall. All rights reserved.</p>
      </div>
    </footer>
  );
}
